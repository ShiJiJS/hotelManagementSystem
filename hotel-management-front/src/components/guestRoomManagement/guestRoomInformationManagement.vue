<template>
  <div>
    ...
    <el-row :gutter="20" style="margin-bottom: 12px">
      <el-col :span="4">
        <el-input style="line-height: 36px" v-model="searchForm.id" placeholder="请输入客房编号"></el-input>
      </el-col>
      <el-col :span="4">
        <el-input style="line-height: 36px" v-model="searchForm.type" placeholder="请输入客房类型"></el-input>
      </el-col>
      <el-col :span="4">
        <el-input style="line-height: 36px" v-model="searchForm.status" placeholder="请输入客房状态"></el-input>
      </el-col>
      <el-col :span="4">
        <el-input style="line-height: 36px" v-model="searchForm.floor" placeholder="请输入楼层"></el-input>
      </el-col>
      <el-col :span="3">
        <el-input style="line-height: 36px" v-model="searchForm.standardPrice" placeholder="请输入标准价格"></el-input>
      </el-col>
      <el-col :span="1">
        <el-button icon="el-icon-search" @click="conditionQuery"></el-button>
      </el-col>
      <el-col :span="2">
        <el-button type="primary" @click="addRoomDialogVisible = true">添加客房</el-button>
      </el-col>
      <el-col :span="2">
    ...
    <el-table :data="tableData" border style="width: 100%" @selection-change="handleSelectionChange">
      ...
      <el-table-column prop="standardPrice" label="标准价格" width="180"></el-table-column>
      <el-table-column prop="discountPrice" label="折扣价格" width="180"></el-table-column>
      <el-table-column prop="standardUserPrice" label="普通会员价格" width="180"></el-table-column>
      <el-table-column prop="vipUserPrice" label="尊贵会员价格" width="180"></el-table-column>
      ...
      <el-form :model="addRoomForm" :rules="RoomRules" ref="addRoomFormRef" label-width="80px">
        ...
        <el-form-item label="类型" prop="type">
          ...
          <el-select v-model="addRoomForm.type" placeholder="请选择类型">
            <el-option v-for="item in roomTypes" :key="item.id" :label="item.type" :value="item.type"></el-option>
          </el-select>
        </el-form-item>
        ...
        <el-form-item label="楼层" prop="floor">
          <el-select v-model="addRoomForm.floor" placeholder="请选择楼层">
            <el-option v-for="item in floors" :key="item.id" :label="item.name" :value="item.name"></el-option>
          </el-select>
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    getTotalPages() {
      var _this = this;
      axios.get(global_.serverSrc + "/rooms/totalPages/" + this.itemsOnOnePage).then((resopnse) => {
        if (resopnse.data.code == global_.GET_OK) {
          _this.pages = resopnse.data.data;
          this.getAllRooms();
        } else {
    ...
    getAllRooms() {
      var _this = this;
      axios.get(global_.serverSrc + "/rooms/" + _this.currentPage + "/" + _this.itemsOnOnePage).then((resopnse) => {
        if (resopnse.data.code == global_.GET_OK) {
          _this.tableData = resopnse.data.data;
        } else {
          alert(resopnse.data.msg);
  ...
  data() {
    return {
      ...
      RoomRules: {
        ...
        standardUserPrice: [{ required: true, message: "请输入标准会员价格", trigger: "blur" }],
        vipUserPrice: [{ required: true, message: "请输入尊贵会员价格", trigger: "blur" }],
      },
      tableData: [],
      //new
      input: "",
