<template>
  <div>
    ...
    <el-row :gutter="20" style="margin-bottom: 12px;">
      <el-col :span="20">
        <el-input style="line-height: 36px;" v-model="searchForm.name" placeholder="请输入商品类别">
          <el-button slot="append" icon="el-icon-search" @click="conditionQuery">
          </el-button>
        </el-input>
      </el-col>
      <el-col :span="2">
        <el-button type="primary" @click="addgoodsTypeDialogVisible = true">
          添加类别
        </el-button>
      </el-col>
      <el-col :span="2">
    ...
    <el-table :data="tableData" border style="width: 100%" @selection-change="handleSelectionChange">
      ...
      <el-table-column prop="id" label="类别代码" width="180">
      </el-table-column>
    ...
    <el-dialog title="添加类别" :visible.sync="addgoodsTypeDialogVisible" width="30%" center>
      <el-form :model="addgoodsTypeForm" :rules="goodsTypeRules" ref="addgoodsTypeFormRef" label-width="80px">
        <!-- <el-form-item label="类别代码" prop="id">
          <el-input v-model="addgoodsTypeForm.id"></el-input>
        </el-form-item> -->
        <el-form-item label="类别名称" prop="name">
          <el-input v-model="addgoodsTypeForm.name"></el-input>
        </el-form-item>
        <el-form-item label="备注" prop="remark">
          <el-input type="textarea" v-model="addgoodsTypeForm.remark"></el-input>
        </el-form-item>
      </el-form>
      <span slot="footer" class="dialog-footer">
        <el-button @click="addgoodsTypeDialogVisible = false">取 消</el-button>
        <el-button type="primary" @click="addgoodsType">确 定</el-button>
      </span>
    </el-dialog>

    <el-dialog title="修改类别" :visible.sync="editgoodsTypeDialogVisible" width="30%" center>
      <el-form :model="editgoodsTypeForm" :rules="goodsTypeRules" ref="editgoodsTypeFormRef" label-width="80px">
        <el-form-item label="类别代码" prop="id">
          <el-input v-model="editgoodsTypeForm.id" disabled></el-input>
        </el-form-item>
        <el-form-item label="类别名称" prop="name">
          <el-input v-model="editgoodsTypeForm.name"></el-input>
        </el-form-item>
        <el-form-item label="备注" prop="remark">
          <el-input type="textarea" v-model="editgoodsTypeForm.remark"></el-input>
        </el-form-item>
      </el-form>
      <span slot="footer" class="dialog-footer">
        <el-button @click="editgoodsTypeDialogVisible = false">取 消</el-button>
        <el-button type="primary" @click="editgoodsType">确 定</el-button>
      </span>
    </el-dialog>
...
export default {
  ...
  methods: {
    ...
    conditionQuery() {
      ...
      if (flag) {
        ...
        axios({
          method: 'post',
          url: global_.serverSrc + '/goodsTypes/search',
          headers: {
    ...
    showEditDialog(id) {
      // 发送请求通过id拿到旧数据，填在表格里
      var _this = this
      axios.get(global_.serverSrc + '/goodsTypes/' + id).then((resopnse) => {
        _this.editgoodsTypeForm = resopnse.data.data
      })
      this.editgoodsTypeDialogVisible = true
    },
    ...
    //按照id批量删除
    deleteByIds() {
      var url = global_.serverSrc + '/goodsTypes/'
      //拼接删除用字符串
    ...
    //打开表格点击确定后添加的请求写在这里
    addgoodsType() {
      this.$refs.addgoodsTypeFormRef.validate((valid) => {
        if (valid) {
          var _this = this
          axios({
            method: 'post',
            url: global_.serverSrc + '/goodsTypes',
            headers: {
              'Content-Type': 'application/json',
            },
            data: JSON.stringify(_this.addgoodsTypeForm),
          }).then((resopnse) => {
          ...
          //刷新列表
          _this.getAllGoodsTypes()
          window.location.reload()
          this.addgoodsTypeDialogVisible = false
        } else {
    ...
    //打开表格点击确定后修改的请求写在这里
    editgoodsType() {
      this.$refs.editgoodsTypeFormRef.validate((valid) => {
        if (valid) {
          var _this = this
          axios({
            method: 'put',
            ...
            data: JSON.stringify(_this.editgoodsTypeForm),
          }).then((resopnse) => {
            if (resopnse.data.code == global_.SAVE_ERR) {
              alert('添加失败')
            }
          ...
          this.getAllGoodsTypes()
          window.location.reload()
          this.editgoodsTypeDialogVisible = false
        } else {
          console.log('error submit!!')
          return false
        }
  ...
  data() {
    return {
      ...
      addgoodsTypeDialogVisible: false,
      addgoodsTypeForm: {
        id: '',
        name: '',
        remark: '',
      },
      editgoodsTypeDialogVisible: false,
      editgoodsTypeForm: {
        id: '',
        name: '',
        remark: '',
      ...
      goodsTypeRules: {
        id: [{ required: true, message: '请输入类别代码', trigger: 'blur' }],
